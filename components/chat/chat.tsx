...
const Chat = ({ chat }: { chat: ChatWithMessageCountAndSettings }) => {
  ...
  return (
    <Link
      ...
      href={`/chat/${chat.id}`}
    >
      <div className={`flex items-center w-full gap-2 px-3 py-2 transition-colors duration-100 ease-in-out rounded-md bg-neutral-100 dark:bg-neutral-900 dark:hover:bg-neutral-800 ${isCurrentChat ? 'font-bold' : 'font-normal'}`}>
